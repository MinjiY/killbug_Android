...
void StartSensor(engine* ap_engine)
{
	...
	if (ap_engine->accelerometer_sensor != NULL) {
			ASensorEventQueue_enableSensor(ap_engine->p_sensor_queue, ap_engine->accelerometer_sensor);

			// 초당 60개의 이벤트를 가져올 수 있습니다.
			ASensorEventQueue_setEventRate(ap_engine->p_sensor_queue, ap_engine->accelerometer_sensor, (1000L / 60) * 1000);
		}
	if (ap_engine->gyroscope_sensor != NULL) {
		ASensorEventQueue_enableSensor(ap_engine->p_sensor_queue, ap_engine->gyroscope_sensor);

		// 초당 60개의 이벤트를 가져올 수 있습니다.
		ASensorEventQueue_setEventRate(ap_engine->p_sensor_queue, ap_engine->gyroscope_sensor, (1000L / 60) * 1000);
	}
	if (ap_engine->magnet_sensor != NULL) {
		ASensorEventQueue_enableSensor(ap_engine->p_sensor_queue, ap_engine->magnet_sensor);

		// 초당 60개의 이벤트를 가져올 수 있습니다.
		ASensorEventQueue_setEventRate(ap_engine->p_sensor_queue, ap_engine->magnet_sensor, (1000L / 60) * 1000);
	}
}

void StopSensor(engine* ap_engine)
{
	if (ap_engine->accelerometer_sensor != NULL) ASensorEventQueue_disableSensor(ap_engine->p_sensor_queue, ap_engine->accelerometer_sensor);
	if (ap_engine->gyroscope_sensor != NULL) ASensorEventQueue_disableSensor(ap_engine->p_sensor_queue, ap_engine->gyroscope_sensor);
	if (ap_engine->magnet_sensor != NULL) ASensorEventQueue_disableSensor(ap_engine->p_sensor_queue, ap_engine->magnet_sensor);
...
static void engine_draw_frame(struct engine* engine) {
	...
	for (y = 0; y < 8; y++) {
		for (x = 0; x < 2; x++) {
			...
			glDrawArrays(GL_TRIANGLE_FAN, 0, 4);			// 두번째네모 

			glColor4f(0.0, 0.05, 0.15, 1);
			SetRectF(pos, -1 + x * 1 + INNER_X_MARGIN+INNER_X_MARGIN/2.0, 1 - y*0.25 - INNER_Y_MARGIN-INNER_Y_MARGIN/2.0,
				-1 + (x + 1) * 1 - INNER_X_MARGIN - INNER_X_MARGIN/2.0, 1 - (y + 1)*0.25 + INNER_Y_MARGIN+INNER_Y_MARGIN/2.0);
			glVertexPointer(2, GL_FLOAT, 0, pos);
			glDrawArrays(GL_TRIANGLE_FAN, 0, 4);			// 가장안쪽네모
...
* 다음 입력 이벤트를 처리합니다.
*/
// 안드로이드는 인풋이 두개 있음 센서는 안드로이드메인 그외동작은 handle
...
void android_main(struct android_app* state) {
	...
	engine.accelerometer_sensor = ASensorManager_getDefaultSensor(engine.sensorManager, ASENSOR_TYPE_ACCELEROMETER);
	...
	srv_addr.sin_addr.s_addr = inet_addr("192.168.0.132");
